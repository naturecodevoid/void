 ...
 * @returns
 */
export function fitText(text, originalSize, textElement, outerElementWidth, increment = 0.25, threshold = 0.5) {
    if ("value" in textElement)
        textElement.value = text;
    else
        textElement.textContent = text;
    if (Math.abs(textElement.getBoundingClientRect().width - outerElementWidth) < threshold)
        return console.debug("skipping because of threshold");
    textElement.style.fontSize = originalSize;
    textElement.style.width = "unset";
    if (textElement.getBoundingClientRect().width < outerElementWidth)
        return console.debug("text fits in outer element");
